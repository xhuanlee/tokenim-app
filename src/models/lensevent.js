import { fetchMoreRoom, fetchUser, isHost, saveChatRoom, saveUser } from '@/service/lensprotocol';
...
export default {
  ...
  effects: {
    *fetchMore(_, { call, put, select }) {
      const meetingServer = yield select(state => state.lensevent.meetingServer);
      console.log("server 1:"+meetingServer);
      const page = yield select(state => state.lensevent.page);
      const response = yield call(fetchMoreRoom, meetingServer, page, PAGE_SIZE);
      const { code, data } = response;
      if (code && code == 200) {
        const oldRooms = yield select(state => state.lensevent.rooms);
